...
class MGraph__Index__Types(Type_Safe):
    ...
    def index_node_type(self, node_id: Node_Id, node_type_name: str) -> None:                # Index a node's type
        self.index_data.nodes_types[node_id] = node_type_name                                # Store node_id to type mapping

        if node_type_name not in self.index_data.nodes_by_type:                              # Store type to node_ids mapping
    ...
    def remove_node_type(self, node_id: Node_Id, node_type_name: str) -> None:               # Remove a node's type from index
        if node_id in self.index_data.nodes_types:                                           # Remove from nodes_types mapping
            del self.index_data.nodes_types[node_id]

        if node_type_name in self.index_data.nodes_by_type:                                  # Remove from nodes_by_type mapping
            self.index_data.nodes_by_type[node_type_name].discard(node_id)
            if not self.index_data.nodes_by_type[node_type_name]:                            # Clean up empty sets
    ...
    def index_edge_type(self, edge_id     : Edge_Id ,                                        # Index an edge's type
        ...
        self.index_data.edges_types[edge_id] = edge_type_name                                # Store edge_id to type mapping

        if edge_type_name not in self.index_data.edges_by_type:                              # Store type to edge_ids mapping
    ...
    def _index_edge_type_by_node(self, node_id       : Node_Id ,                             # Helper to index edge type by node
                                       ...
                                       incoming      : bool    ) -> None:
        if incoming:
            edges_by_type = self.index_data.nodes_to_incoming_edges_by_type
        else:
            edges_by_type = self.index_data.nodes_to_outgoing_edges_by_type
    ...
    def remove_edge_type(self, edge_id: Edge_Id, edge_type_name: str) -> None:               # Remove an edge's type from edges_by_type
        if edge_type_name and edge_type_name in self.index_data.edges_by_type:
            ...
            if not self.index_data.edges_by_type[edge_type_name]:                            # Clean up empty sets
    ...
    def get_node_type(self, node_id: Node_Id) -> Optional[str]:                              # Get type name for a specific node
        return self.index_data.nodes_types.get(node_id)
    ...
    def get_edge_type(self, edge_id: Edge_Id) -> Optional[str]:                              # Get type name for a specific edge
        return self.index_data.edges_types.get(edge_id)
    ...
    def get_node_incoming_edges_by_type(self, node_id  : Node_Id ,                           # Get incoming edges for a node filtered by type
        ...
        return self.index_data.nodes_to_incoming_edges_by_type.get(node_id, {}).get(type_name, set())

    def get_node_outgoing_edges_by_type(self, node_id  : Node_Id ,                           # Get outgoing edges for a node filtered by type
                                              type_name: str
                                         ) -> Set[Edge_Id]:
        return self.index_data.nodes_to_outgoing_edges_by_type.get(node_id, {}).get(type_name, set())

    def get_node_edge_types_incoming(self, node_id: Node_Id) -> Set[str]:                    # Get all edge types incoming to a node
        return set(self.index_data.nodes_to_incoming_edges_by_type.get(node_id, {}).keys())

    def get_node_edge_types_outgoing(self, node_id: Node_Id) -> Set[str]:                    # Get all edge types outgoing from a node
        return set(self.index_data.nodes_to_outgoing_edges_by_type.get(node_id, {}).keys())
