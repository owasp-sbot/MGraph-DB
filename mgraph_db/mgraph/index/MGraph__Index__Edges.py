...
class MGraph__Index__Edges(Type_Safe):
    ...
    def init_node_edge_sets(self, node_id: Node_Id) -> None:                                 # Initialize empty edge sets for a node
        if node_id not in self.index_data.nodes_to_outgoing_edges:
    ...
    def index_edge(self, edge_id     : Edge_Id ,                                             # Index an edge's node connections
        ...
        if from_node_id not in self.index_data.nodes_to_outgoing_edges:                      # Add to outgoing edges
            self.index_data.nodes_to_outgoing_edges[from_node_id] = set()
        self.index_data.nodes_to_outgoing_edges[from_node_id].add(edge_id)

        if to_node_id not in self.index_data.nodes_to_incoming_edges:                        # Add to incoming edges
    ...
    def get_edge_nodes(self, edge_id: Edge_Id) -> Optional[Tuple[Node_Id, Node_Id]]:         # Get edge endpoints (from, to)
        return self.index_data.edges_to_nodes.get(edge_id)

    def get_edge_from_node(self, edge_id: Edge_Id) -> Optional[Node_Id]:                     # Get edge source node
        result = self.index_data.edges_to_nodes.get(edge_id)
    ...
    def has_edge(self, edge_id: Edge_Id) -> bool:                                            # Check if edge exists
        return edge_id in self.index_data.edges_to_nodes

    def edge_count(self) -> int:                                                             # Get total edge count
        return len(self.index_data.edges_to_nodes)
    ...
    def get_node_all_edges(self, node_id: Node_Id) -> Set[Edge_Id]:                          # Get all edges connected to a node
        outgoing = self.index_data.nodes_to_outgoing_edges.get(node_id, set())
        incoming = self.index_data.nodes_to_incoming_edges.get(node_id, set())
        return outgoing | incoming

    def count_node_outgoing_edges(self, node_id: Node_Id) -> int:                            # Count outgoing edges
        return len(self.index_data.nodes_to_outgoing_edges.get(node_id, set()))

    def count_node_incoming_edges(self, node_id: Node_Id) -> int:                            # Count incoming edges
        return len(self.index_data.nodes_to_incoming_edges.get(node_id, set()))

    def has_node_outgoing_edges(self, node_id: Node_Id) -> bool:                             # Check if node has outgoing edges
        return bool(self.index_data.nodes_to_outgoing_edges.get(node_id))

    def has_node_incoming_edges(self, node_id: Node_Id) -> bool:                             # Check if node has incoming edges
        return bool(self.index_data.nodes_to_incoming_edges.get(node_id))
    ...
    def get_connected_nodes_outgoing(self, node_id: Node_Id) -> Set[Node_Id]:                # Get nodes connected via outgoing edges
        result = set()
        for edge_id in self.index_data.nodes_to_outgoing_edges.get(node_id, set()):
    ...
    def get_connected_nodes_incoming(self, node_id: Node_Id) -> Set[Node_Id]:                # Get nodes connected via incoming edges
        result = set()
        for edge_id in self.index_data.nodes_to_incoming_edges.get(node_id, set()):
            edge_nodes = self.index_data.edges_to_nodes.get(edge_id)
            if edge_nodes:
    ...
    def edges_ids__from__node_id(self, node_id: Node_Id) -> List[Edge_Id]:                   # Get outgoing edge IDs as list
        return list(self.index_data.nodes_to_outgoing_edges.get(node_id, set()))

    def edges_ids__to__node_id(self, node_id: Node_Id) -> List[Edge_Id]:                     # Get incoming edge IDs as list
        return list(self.index_data.nodes_to_incoming_edges.get(node_id, set()))
