...
class test_MGraph__Index__Coverage(TestCase):
    ...
    def test_remove_edge_by_id__without_type(self):                             # Test remove_edge_by_id when edge_type_name is None
        with self.mgraph_index as _:
            ...
            del _.index_data.edges_types[edge.edge_id]                          # Manually remove type to simulate edge case

            result = _.remove_edge_by_id(edge.edge_id)                          # Should still work without crashing

            assert result is _                                                  # Returns self for chaining
            assert edge.edge_id not in _.index_data.edges_to_nodes
    ...
    def test_remove_edge_by_id__edge_not_in_edges_to_nodes(self):               # Test when edge_id not in edges_to_nodes
        with self.mgraph_index as _:
            ...
            del _.index_data.edges_to_nodes[edge.edge_id]                       # Remove from edges_to_nodes manually
    ...
    def test__remove_edge_path_by_id__edge_not_in_any_path(self):               # Test when edge has no path
        with self.mgraph_index as _:
            ...
            assert edge.edge_id not in _.index_data.edges_by_path               # Still empty
    ...
    def test__remove_edge_path_by_id__cleans_empty_set(self):                   # Test cleanup of empty path set
        with self.mgraph_index as _:
            ...
            assert Edge_Path("solo.path") in _.index_data.edges_by_path

            _._remove_edge_path_by_id(edge.edge_id)

            assert Edge_Path("solo.path") not in _.index_data.edges_by_path     # Empty set should be cleaned up
    ...
    def test__remove_edge_label_by_id__partial_labels(self):                    # Test when edge has only some labels
        with self.mgraph_index as _:
            ...
            assert Safe_Id('only_pred') in _.index_data.edges_by_predicate

            _._remove_edge_label_by_id(edge.edge_id)

            assert Safe_Id('only_pred') not in _.index_data.edges_by_predicate
    ...
    def test__remove_edge_label_by_id__all_label_types(self):                   # Test removal of all label types
        with self.mgraph_index as _:
            ...
            assert Safe_Id('pred'    ) not in _.index_data.edges_by_predicate
            assert Safe_Id('incoming') not in _.index_data.edges_by_incoming_label
            assert Safe_Id('outgoing') not in _.index_data.edges_by_outgoing_label
    ...
    def test_remove_edge_label__with_all_labels(self):                          # Test remove_edge_label method directly
        with self.mgraph_index as _:
            ...
            assert Safe_Id('to_remove'         ) not in _.index_data.edges_by_predicate
            assert Safe_Id('incoming_to_remove') not in _.index_data.edges_by_incoming_label
            assert Safe_Id('outgoing_to_remove') not in _.index_data.edges_by_outgoing_label
    ...
    def test_remove_edge_label__only_predicate(self):                           # Test with only predicate label
        with self.mgraph_index as _:
            ...
            assert Safe_Id('just_pred') not in _.index_data.edges_by_predicate
    ...
    def test_remove_edge_label__predicate_not_in_index(self):                   # Test when predicate already removed from index
        with self.mgraph_index as _:
            ...
            del _.index_data.edges_by_predicate[Safe_Id('orphan_pred')]         # Manually remove
    ...
    def test_remove_edge_path__with_path(self):                                 # Test remove_edge_path method
        with self.mgraph_index as _:
            ...
            assert Edge_Path("to.remove") in _.index_data.edges_by_path

            _.remove_edge_path(edge)

            assert Edge_Path("to.remove") not in _.index_data.edges_by_path
    ...
    def test_remove_edge_path__path_not_in_index(self):                         # Test when path already removed
        with self.mgraph_index as _:
            ...
            del _.index_data.edges_by_path[Edge_Path("orphan.path")]            # Manually remove
    ...
    def test_remove_node__with_outgoing_edges(self):                            # Test node removal cleans up outgoing edges
        with self.mgraph_index as _:
            ...
            assert node_1.node_id not in _.index_data.nodes_to_outgoing_edges
            assert edge_1.edge_id not in _.index_data.edges_to_nodes
            assert edge_2.edge_id not in _.index_data.edges_to_nodes
    ...
    def test_remove_node__with_incoming_edges(self):                            # Test node removal cleans up incoming edges
        with self.mgraph_index as _:
            ...
            assert node_3.node_id not in _.index_data.nodes_to_incoming_edges
            assert edge_1.edge_id not in _.index_data.edges_to_nodes
            assert edge_2.edge_id not in _.index_data.edges_to_nodes
    ...
    def test_remove_node__cleans_type_index(self):                              # Test type index cleanup
        with self.mgraph_index as _:
            ...
            assert 'Schema__MGraph__Node' in _.index_data.nodes_by_type

            _.remove_node(node)

            assert 'Schema__MGraph__Node' not in _.index_data.nodes_by_type     # Type entry removed when empty
    ...
    def test_save_to_file__and_from_file(self):                                 # Test save and load roundtrip
        with self.mgraph_index as _:
            ...
            with Temp_File(return_file_path=True, extension='json', create_file=False) as target_file:
                ...
                assert len(loaded_index.index_data.edges_to_nodes) == 1
                assert len(loaded_index.index_data.nodes_by_type)  == 1
    ...
    def test_add_node__with_value_node(self):                                   # Test adding value node to main index
        with self.mgraph_index as _:
            ...
            assert value_node.node_id in _.index_data.nodes_types
